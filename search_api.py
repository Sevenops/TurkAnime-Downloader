...
def decode_iframe(a):
    ...
    a = {
        "ct": "P5rh7okRVe0uBsN+M8LCi11SFSnojXxDAv88ZX7TQtMYslz0OAWF+KPAA2d1uQES",
        "iv": "94d71d7bc0975ec13724edabf2fe1885",
        "s": "48a8c852f99b1ee6",
    }
...
class TurkAnime:
    ...
    def __init__(self):
        if not cookies.get("Nekil"):
            veri = httpx.get(self.url, headers=headers, cookies=cookies).content
            r = re.search("\|([0-9a-f]{30,50})\|", veri.decode("utf8"))
            if r:
                TurkAnime.nekilCookie = r.group(1)
                cookies["Nekil"] = TurkAnime.nekilCookie
            else:
                print("Nekil kodu bulmada hata var.")
